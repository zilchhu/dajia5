...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$http.get(api.MT_SPAREAS + "?" + times)
        .then(res => {
          console.log(res)
    ...
    updateBase(dateSours) {
      ...
      dateSours.forEach(function (value) {
        ...
        let logss = value['logisticsareas']
        logss = logss.replace(reg, "")
        logisticsareas.push(logss)
        minprice.push(value['minprice'])
        shippingfee.push(value['shippingfee'])
      })
